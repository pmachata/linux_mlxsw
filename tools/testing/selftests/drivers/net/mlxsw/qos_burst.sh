...
# xxx: clean up the test_XXX mess down there
# xxx: don't assume 60M pool can be created, deduce from devlink-reported SB size
...
switch_create()
{
	...
	# Burst soak pools.
	devlink_pool_size_thtype_set 2 dynamic $POOL_SIZE
	devlink_pool_size_thtype_set 6 dynamic $POOL_SIZE
	...
	devlink_port_pool_th_set $swp2 6 16
	devlink_tc_bind_pool_th_set $swp2 1 egress 6 16
	...
	devlink_port_pool_th_set $swp4 2 16
	devlink_tc_bind_pool_th_set $swp4 1 ingress 2 16
	...
	devlink_port_pool_th_set $swp5 2 16
	devlink_tc_bind_pool_th_set $swp5 1 ingress 2 16
...
__test_qos_burst()
{
	...
	start_traffic $h1.111 192.0.2.33 192.0.2.34 $h2mac $pktsize
	sleep 1
	...
	# Threshold 16 means alpha 64. How much traffic gets admitted:
	local admit_cells=$((cells * 64 / 65))
	# Each packet is 8K of payload + headers.
	local pkt_cells=$(((pktsize + 50 + cell_size - 1)  / cell_size))
	# How many packets can we admit:
	local pkts=$((admit_cells / pkt_cells))
	...
	# Check that the queue is somewhat close to 10MB. This makes sure that
	# the lack of drops above was not due to port undersubscribtion.
	local q0=$(ethtool_stats_get $swp2 tc_transmit_queue_tc_1)
...
test_4K()
{
	__test_qos_burst 4000
}

test_2K()
{
	__test_qos_burst 2000
}

test_1500()
{
	__test_qos_burst 1500
}

test_1K()
{
	__test_qos_burst 1000
}

test_750()
{
	__test_qos_burst 750
}

test_688()
{
	__test_qos_burst 688
}

test_672()
{
	__test_qos_burst 672
}

test_671()
{
	__test_qos_burst 671
}

test_670()
{
	__test_qos_burst 670
}

test_669()
{
	__test_qos_burst 669
}

test_668()
{
	__test_qos_burst 668
}

test_664()
{
	__test_qos_burst 664
}

test_656()
{
	__test_qos_burst 656
}

test_625()
{
	__test_qos_burst 625
}

test_500()
{
	__test_qos_burst 500
}

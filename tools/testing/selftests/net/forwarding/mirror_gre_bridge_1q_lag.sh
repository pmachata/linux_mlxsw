...
# bridge device with vlan filtering (802.1q), and the egress device is a LAG
# (bond or team) device.
...
h4_create()
{
	simple_if_init $h4 192.0.2.130/28 2001:db8:2::2/64
...
h4_destroy()
{
	...
	simple_if_fini $h4 192.0.2.130/28 2001:db8:2::2/64
}
...
switch_create()
{
	...
	__addr_add_del br1 add 192.0.2.129/32 2001:db8:2::1/128
...
switch_destroy()
{
	...
	ip -6 route del 2001:db8:2::2/128 dev br1
	ip -4 route del 192.0.2.130/32 dev br1
	__addr_add_del br1 del 192.0.2.129/32 2001:db8:2::1/128

...
void __devlink_nl_post_doit(const struct genl_split_ops *ops,
			    struct sk_buff *skb, struct genl_info *info,
			    u8 flags)
{
	struct devlink *devlink;
...
void devlink_nl_post_doit(const struct genl_split_ops *ops,
			  struct sk_buff *skb, struct genl_info *info)
{
	__devlink_nl_post_doit(ops, skb, info, 0);
}

void devlink_nl_post_doit_dev_lock(const struct genl_split_ops *ops,
				   struct sk_buff *skb, struct genl_info *info)
{
	__devlink_nl_post_doit(ops, skb, info, DEVLINK_NL_FLAG_NEED_DEV_LOCK);
}

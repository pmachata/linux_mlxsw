				    ...
				    struct bpf_prog *prog)
{
	...
	mlxsw_core_xdp_port_prog_set(mlxsw_sp->core, mlxsw_sp_port->local_port,
				     prog);
}
				...
				struct mlxsw_sp_port_mapping *port_mapping)
{
	...
	err = mlxsw_core_xdp_port_init(mlxsw_sp->core, local_port, dev);
	if (err)
		goto err_xdp_port_init;
...
err_alloc_stats:
	mlxsw_core_xdp_port_fini(mlxsw_sp->core, local_port);
err_xdp_port_init:
	free_netdev(dev);
err_alloc_etherdev:

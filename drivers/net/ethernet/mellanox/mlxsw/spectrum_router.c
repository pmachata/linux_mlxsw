				 ...
				 struct netlink_ext_ack *extack)
{
	...
	// XXX no need to migrate next hops, they are kept at CRIF, not RIF, and
	// that stays intact. But we need to update the next hops to use the new
	// RIF. Need to do this here before destroying the old RIF.
	mlxsw_sp_nexthop_rif_update(mlxsw_sp, &new_lb_rif->common);

	// XXX Now destroy the old RIF, which destroys the CRIF-RIF link. Using
	// the same CRIF for another RIF is odd and something that should
	// perhaps be warned on, so it's odd we so brazenly just walk right into
	// this configuration. Looks like this needs some remodeling.
	crif = old_lb_rif->common.crif;
	mlxsw_sp_rif_destroy(&old_lb_rif->common);

	new_lb_rif->common.crif = crif;
	crif->rif = &new_lb_rif->common;
			     ...
			     struct mlxsw_sp_neigh_entry *neigh_entry)
{
	if (WARN_ON(!neigh_entry->crif->rif))
		// xxx consider splitting neighbor entries to cneighs and
		// rneighs, along the lines of CRIFs and RIFs
		return;
...
mlxsw_sp_neigh_entry_create(struct mlxsw_sp *mlxsw_sp, struct neighbour *n)
{
	...
	printk(KERN_WARNING "neigh_create %s %pM\n",
	       n->dev->name, n->ha);
	return neigh_entry;
...
mlxsw_sp_neigh_entry_destroy(struct mlxsw_sp *mlxsw_sp,
			     struct mlxsw_sp_neigh_entry *neigh_entry)
{
	printk(KERN_WARNING "neigh_destroy %s %pM\n",
	       neigh_entry->crif->key.dev->name, neigh_entry->ha);
	list_del(&neigh_entry->crif_list_node);

	WARN_ON(!!neigh_entry->crif->rif && neigh_entry->counter_valid);
	if (neigh_entry->crif->rif) {
...
static void mlxsw_sp_router_neigh_event_work(struct work_struct *work)
{
	...
	// xxx this then calls neigh OP without RIF
					  ...
					  unsigned long event, void *ptr)
{
	...
	case NETEVENT_NEIGH_UPDATE:
		 ...
		 */
		neigh_clone(n);
		return mlxsw_sp_router_schedule_work(net, router,
				mlxsw_sp_router_neigh_event_work, n);

	case NETEVENT_IPV4_MPATH_HASH_UPDATE:
	case NETEVENT_IPV6_MPATH_HASH_UPDATE:
		return mlxsw_sp_router_schedule_work(ptr, router,
				mlxsw_sp_router_mp_hash_event_work, NULL);

	case NETEVENT_IPV4_FWD_UPDATE_PRIORITY_UPDATE:
		return mlxsw_sp_router_schedule_work(ptr, router,
				mlxsw_sp_router_update_priority_work, NULL);
	}
					...
					struct mlxsw_sp_rif *rif)
{
	...
	list_for_each_entry_safe(neigh_entry, tmp, &rif->crif->neigh_list,
				 crif_list_node) {
		printk(KERN_WARNING "neigh: RIF made (%s) xxx\n",
		       mlxsw_sp_rif_dev(rif)->name);
		/*
		mlxsw_sp_neigh_entry_update(mlxsw_sp, neigh_entry, true);
		mlxsw_sp_neigh_entry_destroy(mlxsw_sp, neigh_entry);
		*/
...
mlxsw_sp_nexthop_netdev(const struct mlxsw_sp_nexthop *nh)
{
	if (nh->crif) // xxx should this always hold?
...
static struct mlxsw_sp_rif *
mlxsw_sp_nhgi_rif(struct mlxsw_sp_nexthop_group_info *nhgi)
{
	struct mlxsw_sp_crif *crif = nhgi->nexthops[0].crif;

	return crif ? crif->rif : NULL;
}
		     ...
		     const struct mlxsw_sp_rt6 *mlxsw_sp_rt6)
{
	...
	for (i = 0; i < nh_grp->nhgi->count; i++) {
		...
		if (mlxsw_sp_nexthop_netdev(nh) == rt->fib6_nh->fib_nh_dev &&
...
static bool mlxsw_sp_dev_addr_list_empty(struct net_device *dev)
{
				     ...
				     struct netdev_notifier_changeupper_info *info)
{
	...
	addr_list_empty = mlxsw_sp_dev_addr_list_empty(upper_dev);

	crif = mlxsw_sp_crif_lookup(mlxsw_sp->router, upper_dev);
	if (WARN_ON(!crif))
		return -EINVAL;

	// xxx this needs to walk uppers as well. Enslaving to a LAG can cause
	// creation of a RIF on its master bridge, and on that bridge's VLAN.

	if (info->linking && !addr_list_empty && !crif->rif) {
		struct mlxsw_sp_rif_params params = {
			.dev = upper_dev,
		};
		struct mlxsw_sp_rif *rif;
		...
		printk(KERN_WARNING "creating RIF for %s\n", upper_dev->name);
		rif = mlxsw_sp_rif_create(mlxsw_sp, &params, extack);
		if (IS_ERR(rif))
			return PTR_ERR(rif);
	} else if (!info->linking && crif->rif) {
		// xxx wait, how is this currently not handled? The last port to
		// leave the bridge should cause RIF removal even now...?

		// xxx +if (this was the last port)
	}
					   ...
					   unsigned long event, void *ptr)
{
	...
	if (mlxsw_sp_is_offload_xstats_event(event))
		...
		err = mlxsw_sp_netdevice_enslavement_event(mlxsw_sp, dev, ptr);
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	return mlxsw_sp_fid_8021d_get(rif->mlxsw_sp, dev->ifindex);
}
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	router->inet6addr_valid_nb.notifier_call = mlxsw_sp_inet6addr_valid_event;

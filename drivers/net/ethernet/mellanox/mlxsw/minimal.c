			...
			struct netlink_ext_ack *extack)
{
	...
	/* Take 'devlink' lock to protect mlxsw_m_linecard_ports_create() call
	 * that might be activated by got_active() callback from
	 * mlxsw_linecards_event_ops_register(). This lock must be taken before
	 * and released after 'linecard->lock' to avoid circular locking
